...
struct HeatmapView: View {
    /// In React this would be props: `function HeatmapView({ habit }) { ... }`
    let habit: HabitModel

    // 7 columns = 1 week wide, it will auto-wrap days downwards
    ...
    private func squareColor(for day: HabitDay) -> Color {
        ...
        // Later we can use more shades based on streaks/intensity.
...
#Preview {
    HeatmapView(habit: HabitDemoData.makeSampleHabit())
}
